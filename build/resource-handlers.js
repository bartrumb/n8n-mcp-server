 ...
 */
export function setupResourceHandlers(server, clients) {
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        ...
        if (uri === 'n8n://workflows') {
            try {
                ...
                if (!client) {
                    throw new McpError(ErrorCode.InternalError, 'No n8n client available. Please initialize a client first.');
                }
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to fetch workflows: ${error.message || String(error)}`);
            }
        }
        ...
        if (workflowMatch) {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to fetch workflow ${workflowId}: ${error.message || String(error)}`);
            }
        }
        ...
        if (executionMatch) {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to fetch execution ${executionId}: ${error.message || String(error)}`);
            }
        }
        throw new McpError(ErrorCode.InvalidRequest, `Invalid URI format: ${uri}. Supported formats: n8n://workflows, n8n://workflows/{id}, n8n://executions/{id}`);
    });
}
